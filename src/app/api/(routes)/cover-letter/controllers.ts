...
import { googleDocs, googleDrive } from "../../(utils)/google";
import { DownloadCoverLetterRequestType } from "./validators";
...
export const prepareCoverLetterFile = async ({
  companyName,
  letterContent,
}: DownloadCoverLetterRequestType) => {
  const templateDocId = process.env.GOOGLE_TEMPLATE_COVER_LETTER_DOC_ID!;
